...
const { Plugin, MarkdownView, MarkdownEditView, FileView, EditableFileView, Notice, Platform, setIcon } = require("obsidian");
...
class ReadingHighlighterPlugin extends Plugin {
  ...
  handleSelectionChange() {
    ...
    this.selectionFallback = snippet.trim() || ""

    const isString = typeof sel === "string"

    if(sel && isString ? sel.trim() : sel.toString().trim()) {
  ...
  async highlightSelection(view) {
    const sel = document.getSelection() || view.getSelection()

    /*
      iPhone loses selection on button click, so store the selection
      and use that if the selection is empty.
    */
    const snippet = typeof sel === "string" ? sel : sel?.toString() || this.selectionFallback || ""
    ...
    const a1 = this.posViaSourcePos(sel?.anchorNode || document.activeElement);
    const b1 = this.posViaSourcePos(sel?.focusNode || document.activeElement);
  ...
  posViaSourcePos(node) {
    if (!node) return null;
    /*
      The `while` loop wasn't working, so I replaced it with a
      recursize function.
    */
    function findParent(node) {
      if(node.nodeType === Node.TEXT_NODE) return findParent(node.parentElement)
      return node
    }
    const el = findParent(node)
  ...
  uniqueDirectMatch(src, text) {
    ...
    /*
      The following line guards against documents that have multiple
      instances of the same highlighted string, but the disambiguation
      logic seems to throw errors.
    */
